...
function calc() {
        ...
        elements.forEach(elem => {
            elem.addEventListener('click', (e) => {
                if (e.target.getAttribute('data-ratio')) {
                    //если у блока есть искомый атрибут те работаем с нижним полем выбора
                    ratio = +e.target.getAttribute('data-ratio'); //вытаскиваем начение из дата атрибута
                    localStorage.setItem('ratio', +e.target.getAttribute('data-ratio'));
                } else {
                    //если кликаем на выбор пола
                    sex = e.target.getAttribute('id'); //женщина или мужчина
    ...
    function getDynamicInformation(selector) {
        ...
        input.addEventListener('input', () => {
            if (input.value.match(/\D/g)) { //введено не число
...
class MenuCard {
    constructor(src, alt, title, descr, price, parentSelector, ...classes) {
        ...
        this.parent = document.querySelector(parentSelector); //лежит тут дом эл-т
        this.transfer = 27; //курс доллара для гривен
        this.changeToUAH();

    }

    changeToUAH() { //переводим $ в гривны
...
window.addEventListener('DOMContentLoaded', () => { //скрипт сработает когда сформируется дом-дерево
    ...
    const modalTimerId = setTimeout(() => (0,_modules_modal__WEBPACK_IMPORTED_MODULE_1__.openModalWindow)('.modal', modalTimerId), 5000); //открыть модалку через 5с

    (0,_modules_tabs__WEBPACK_IMPORTED_MODULE_0__.default)('.tabheader__item', '.tabcontent', '.tabheader__items', 'tabheader__item_active');
    (0,_modules_modal__WEBPACK_IMPORTED_MODULE_1__.default)('[data-modal]', '.modal', modalTimerId);
    (0,_modules_timer__WEBPACK_IMPORTED_MODULE_2__.default)('.timer', '2021-01-15');
